...
-   cut -- removes sections (bytes, characters, or fields) from input. See cut --help for switch options. For these examples create a file (columns.txt) with the following command (``t` = TAB character, `\n` = newline character):
